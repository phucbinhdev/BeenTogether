...
import { getCustomQuestions, saveCustomQuestion } from '../services/storageService';
...
export const CardGame: React.FC = () => {
  ...
  const [newQuestionCategory, setNewQuestionCategory] = useState<'fun' | 'deep' | 'spicy'>('fun');
  ...
  const handleNext = () => {
    ...
    setTurn((prev) => prev === PlayerTurn.PLAYER_1 ? PlayerTurn.PLAYER_2 : PlayerTurn.PLAYER_1);
  };
  ...
  const handleAddQuestion = () => {
    ...
    const updatedDeck = saveCustomQuestion(newQuestion);
    setDeck([...QUESTIONS, ...updatedDeck]);
  ...
  const renderCard = (index: number, isTop: boolean) => {
    ...
    const categoryColor = 
      question.category === 'deep' ? 'bg-rose-100 text-rose-500' : 
      question.category === 'fun' ? 'bg-blue-100 text-blue-500' : 
    ...
    return (
      ...
      >
         {/* Badge */}
         <span className={`mb-8 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest ${categoryColor}`}>
            {question.isCustom ? 'Tự tạo • ' : ''}{question.category}
         </span>
    ...
    >
      ...
      <div className="absolute top-4 w-full max-w-sm flex justify-between items-start z-40 px-2">
        <div className="flex flex-col">
            <h2 className="text-xl font-serif font-bold text-slate-800">Lượt của {turn}</h2>
            <span className="text-sm text-slate-400 font-medium mt-1">{currentCardIndex + 1} / {deck.length}</span>
        </div>
        <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
                    <h3 className="font-bold text-slate-800 text-lg">Thêm câu hỏi mới</h3>
                ...
                <div className="p-6 space-y-5">
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-wide">Nội dung câu hỏi</label>
                        <textarea 
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-wide">Chủ đề</label>
